...
def PrintVmInfo(vm, content, vchtime, interval, perf_dict, ):
    ...
    summary = vm.summary
    #print('In PrintVmInfo - vm is %s content is %s vchtime is %s interval is %s perf_dict is %s \n' % (vm, content, vchtime, interval, perf_dict))
    print('In PrintVmInfo - vm is %s \n' % (vm))
    disk_list = []
    network_list = []

    """
    # Convert limit and reservation values from -1 to None
    resCfg = vm.resourceConfig
    print('resCfg is %s \n ' % (resCfg))
    print('vm.resourceConfig.cpuAllocation.limit is %s \n' % (vm.resourceConfig.cpuAllocation.limit))
    ...
    print('After vm.resourceConfig stuff')

    vm_hardware = vm.config.hardware
    print(' vm_hardware is %s \n' % (vm_hardware))
